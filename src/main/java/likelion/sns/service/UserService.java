...
public class UserService {
    ...
    public UserJoinResponseDto createUser(UserJoinRequestDto joinRequest) throws SQLException  {
        ...
        userRepository.findByUserName(joinRequest.getUserName())
                .ifPresent(user -> {throw new SNSAppException(ErrorCode.DUPLICATED_USER_NAME, joinRequest.getUserName()+"는(은) 이미 존재합니다.");});
    ...
    public UserLoginResponseDto loginUser(UserLoginRequestDto loginRequest) throws SQLException {
        ...
        User found = userRepository.findByUserName(inputUsername)
                .orElseThrow(() -> new SNSAppException(ErrorCode.USERNAME_NOT_FOUND, inputUsername+ "에 해당하는 회원을 찾을 수 없습니다."));
